...
├── signals.html           # 訊號紀錄
├── macro.html             # 總經分析
├── admin_daily.html       # 後台頁面（夥伴4 市場分析）
├── requirements.txt
├── .python-version        # 3.12.9（固定給 Railway/mise）
└── automation/
    ├── orchestrator.py    # 每日自動化流水線（夥伴4→夥伴3→Telegram）
    └── telegram_bot.py    # Telegram Bot Webhook 12 個指令 handler
```

**技術棧**
- 後端：Python Flask + PostgreSQL（pg8000）+ gunicorn，部署在 Railway
- 前端：Vanilla HTML / CSS / JS，無框架
- AI：OpenAI GPT-4o（Orchestrator + Telegram Bot 所有 AI）；Claude claude-sonnet-4-6（圖表視覺分析串流）
- 市場資料：Twelve Data（XAU/USD + DXY 即時報價）+ yfinance（VIX/BTC/US100/US10Y/US2Y）
- GitHub：`a0983244497-png/xau-elite-website`
- 部署網址：`https://xau-elite-website-production.up.railway.app`
...
| Table | 用途 |
|---|---|
| `daily_analysis` | 舊系統每日結構分析（方向/偏向/總經/事件/新聞） |
| `orchestrator_articles` | Orchestrator 夥伴4 產出的市場分析文章 |
| `social_drafts` | Orchestrator 夥伴3 產出的 Threads 草稿（含 `is_published`） |
| `market_articles` | 舊夥伴4 文章（Gino 口語風格，獨立排程） |
| `drafts` | 舊夥伴3 草稿（Gino 口語風格，獨立排程） |
| `signals` | 訊號紀錄 |
| `applications` | 學員申請 |
| `strategy_articles` | 夥伴2 策略文章 |
| `feed_items` | 素材收集 |
...
- 網址：`https://xau-elite-website-production.up.railway.app`
- 後台：`/admin/daily`（密碼：`xauelite2024`）
- 環境變數（Railway 設定）：
  - `DATABASE_URL` — PostgreSQL 連線字串
  - `OPENAI_API_KEY` — OpenAI API（Orchestrator + Telegram Bot 主要 AI）
  - `ANTHROPIC_API_KEY` — Claude API（圖表分析串流用）
  - `TWELVE_DATA_KEY` — Twelve Data API（XAU/USD + DXY 即時報價）
  - `ADMIN_KEY` — 後台管理金鑰（`xauelite2024`）
  - `TELEGRAM_BOT_TOKEN` — gino_content_bot token
  - `TELEGRAM_CHAT_ID` — `7260846098`
  - `TELEGRAM_WEBHOOK_SECRET` — `xauelite_tg_2024`
- 靜態 HTML 由 Flask catchall route `/<path:path>` 服務
- Webhook 設定指令（Railway 部署後執行一次）：
  ```bash
  curl -X POST https://xau-elite-website-production.up.railway.app/api/telegram/set-webhook \
    -H "X-Admin-Key: xauelite2024" \
    -H "Content-Type: application/json" \
    -d '{"url": "https://xau-elite-website-production.up.railway.app"}'
  ```
...
| 夥伴 | 角色 | 負責範圍 |
|------|------|---------|
| 夥伴1 | 外匯助教 | 學員問答、外匯/黃金基礎教學貼文 |
| 夥伴2 | 策略助教 | 今日操作策略方向（H4/H1/M15 框架） |
| 夥伴3 | 社群內容創作 | Threads 脆貼文、IG 輪播貼文（3 版本） |
| 夥伴4 | 金融市場整合 | 市場分析文章、新聞摘要、風險提示、週報 |
| 夥伴5（本環境） | 程式工程師 | XAU Elite 網站開發、後端 API、自動化 |

**Orchestrator 自動化流程**（每日台灣時間 08:00）：
夥伴4 抓取 XAU/USD + DXY → 產出市場分析文章（250字）→ 夥伴3 產出 3 個版本 Threads 草稿 → 存入 PostgreSQL → 推送到 Telegram
...
| 項目 | 狀態 | 說明 |
|------|------|------|
| Telegram Webhook 設定 | ⏳ 待執行 | 程式已完成，Railway 部署後需執行一次 curl 設定 webhook |
| 圖文推送 | 📋 規劃中 | 先用固定品牌底圖，搭配夥伴4/3 文字推送到 Telegram |
| 夥伴1 串進學員問答 | 📋 規劃中 | 目標平台：Line 或網站 |
| `orchestrator_logs` 表 | 📋 規劃中 | 記錄每次 Orchestrator 執行時間、狀態、產出摘要 |

---

## 已知限制

- Twelve Data：`/quote?symbol=XAU/USD,DXY&apikey=KEY` 批次抓取，response 欄位 `close`、`change`、`percent_change`
- yfinance：`BTC-USD`（BTC）、`^NDX`（US100）、`^VIX`、`^TNX`（US10Y）、`^IRX`（US2Y）
